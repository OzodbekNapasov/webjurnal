...
export default function MonthlyReport({ techSchool }: MonthlyReportProps) {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            // 1. Fetch all groups and filter locally by tech_school (since column may not exist in database)
            ...
            if (lessons) {
                // Sort by date (convert DD.MM.YYYY to comparable values)
    ...
    const handleExportToExcel = () => {
        ...
        // Prepare raw rows for spreadsheet
        ...
        <>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="relative bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl p-5 sm:p-6 w-[95%] sm:w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl text-left">
                        ...
                        <h3 className="text-lg sm:text-xl font-black text-white mb-2 flex items-center gap-2">
                            <span>📊</span> Oylik hisobot shakllantirish
                        </h3>
                        <p className="text-xs text-slate-400 font-bold mb-6">
                            Guruhlar bo'yicha oylik o'tilgan darslar hisoboti va Excel faylga eksport qilish
                        </p>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                    ...
                                    {reportData.length > 0 && (
                                        <div className="bg-slate-950/30 border border-slate-800/80 p-3 rounded-2xl sm:col-span-2 md:col-span-1 flex items-center justify-center">
                                            ...
                                            >
                                                <span>📥</span> Excel faylga yuklash
                                            </button>
                                        </div>
                                    )}
                                </div>
